...
class FrostJournalVi(
    private val frostJournalContext: Context,
    private val frostJournalCallback: FrostJournalCallBack,
    private val frostJournalWindow: Window
) : WebView(frostJournalContext) {
    private var frostJournalFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun frostJournalSetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.frostJournalFileChooserHandler = handler
    }
    ...
    init {
        ...
        webSettings.apply {
            ...
            userAgentString =
                WebSettings.getDefaultUserAgent(frostJournalContext).replace("; wv)", "")
                    .replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        frostJournalContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(
                            frostJournalContext,
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url?.contains("ninecasino") == true) {
                    FrostJournalApplication.frostJournalInputMode =
                        WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN
                    Log.d(
                        FrostJournalApplication.FROST_JOURNAL_MAIN_TAG,
                        "onPageFinished : ${FrostJournalApplication.frostJournalInputMode}"
                    )
                    frostJournalWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
                    frostJournalEnableStableInputScroll()
                } else {
                    FrostJournalApplication.frostJournalInputMode =
                    ...
                    frostJournalWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
                }
            }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                frostJournalFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            ...
            override fun onCreateWindow(
                ...
                frostJournalHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }
    ...
    private fun frostJournalEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    fun frostJournalFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun frostJournalHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView =
                FrostJournalVi(frostJournalContext, frostJournalCallback, frostJournalWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            frostJournalCallback.frostJournalHandleCreateWebWindowRequest(windowWebView)
        }
    }
